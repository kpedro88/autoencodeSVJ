...
# rt.gSystem.Load("{}/libDelphes.so".format(DELPHES_DIR))
# rt.gInterpreter.Declare('#include "{}/modules/Delphes.h"'.format(DELPHES_DIR))
# rt.gInterpreter.Declare('#include "{}/classes/DelphesClasses.h"'.format(DELPHES_DIR))
# rt.gInterpreter.Declare('#include "{}/classes/DelphesFactory.h"'.format(DELPHES_DIR))
# rt.gInterpreter.Declare('#include "{}/external/ExRootAnalysis/ExRootTreeReader.h"'.format(DELPHES_DIR))
...
class Converter:
    ...
    def __init__(
        ...
        self.selections = get_data_dict([selections_path])
        self.input_file_paths = list(self.selections.keys())

        # core tree, add files, add all trees
        self.files = [rf for rf in [rt.TFile(f) for f in self.input_file_paths] if rf.GetListOfKeys().Contains("Delphes")]
    ...
    def get_event_features(self, tree):
    ...
    def save(self, output_file_name=None):
        ...
        self.log("saving h5 data to file {0}".format(output_file_name))
...
if __name__ == "__main__":
    ...
    # elif len(sys.argv) == 0:
        ...
        output_path = "."
        selections_path = "test_selections.txt"
        name = "qcd"
        save_constituents = False
        efp_basis = -1
        n_constituent_particles = 100
        ...
        core = Converter(
            ...
            selections_path,
            name,
            save_constituents=save_constituents,
            energyflow_basis_degree=efp_basis,
            n_constituent_particles=n_constituent_particles
        )
        
        ret = core.convert((0,100))
        core.save("test.h5")
