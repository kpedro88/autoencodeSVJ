...
def getRate(ch, process, ifile):
       hName = ch + "/"+ process
       print("getRate, hist name: ", hName)
       h = ifile.Get(hName)
       h.__class__ = ROOT.TH1F
       
       if h is None:
              print("ERROR -- getRate -- couldn't find histogram: ", hName)
              exit(0)
       
       h.Print()
       
       print("h N entires: ", h.GetEntries())
       
       print("getRate, hist type: ", type(h))
